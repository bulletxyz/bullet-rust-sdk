...
pub fn extract_schema_info() -> SchemaInfo {
    ...
    let structs = raw_structs
        ...
        .map(|s| {
            ...
            let fields =
                resolve_fields(&type_name, &s.fields, types, &wrapper_indices, &enum_indices);
...
fn resolve_fields(
    ...
    let mappings = map::map_fields(context_name, fields, types, wrapper_indices, enum_indices);
    fields
        .iter()
        .zip(mappings)
        .map(|(f, m)| MappedField {
