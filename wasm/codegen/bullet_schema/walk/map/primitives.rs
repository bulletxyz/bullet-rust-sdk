...
pub fn map_primitive(prim: &Primitive) -> ParamMapping {
    let (param_type, conversion) = match prim {
        Primitive::Bool => ("bool".into(), "{v}".into()),
        Primitive::ByteArray { len } => ("Vec<u8>".into(), byte_array_conversion(*len)),
        Primitive::ByteVec => ("Vec<u8>".into(), "{v}".into()),
        Primitive::U8 => ("u8".into(), "{v}".into()),
        Primitive::U16 => ("u16".into(), "{v}".into()),
        Primitive::U32 => ("u32".into(), "{v}".into()),
        Primitive::U64 => ("u64".into(), "{v}".into()),
        Primitive::I16 => ("i16".into(), "{v}".into()),
        Primitive::I64 => ("i64".into(), "{v}".into()),
        Primitive::U128 => ("u128".into(), "{v}".into()),
...
fn byte_array_conversion(len: usize) -> String {
    format!(
